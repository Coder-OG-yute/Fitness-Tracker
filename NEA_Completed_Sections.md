...
Overall, the solution I developed meets the core requirements identified through my research and client interview:

- The program successfully combines exercise and diet tracking into one platform, making it very accessible and easy to use
- Implements user accounts to be able to save logged data and history to be viewed whenever connected to Wi-Fi
- Account created with secure login using validation and password hashing to further secure data if intercepted or hacked
- Generates four progress graphs viewable across daily, weekly, monthly and yearly time periods, this creates a platform that makes tracking progress visual as highlighted in my research and very easy to track progress across multiple fields
- Integrates a food macro database through the FatSecret API that enables logging of vast types of food
- Provides a science-backed tips and advice system
- Ability to create and track goals
- Workout preset system that removes the mundane repetitive task of logging the same exercises again and again if they are consistently the same
- General user-friendly UI with a navbar for overall easy navigation

The program provides a consolidated, visual and informative experience that both my client and the surveys outlined as being absent from existing tools like freeworkoutlog; it performs as a tool rather than just a logging spreadsheet, a core problem I set out to solve.

However, not every feature planned in section 1.4.1 was implemented in full. Custom exercise entries, where a user could add their own exercise not in the exercise list provided, was initially planned but not built, which is a limitation given that the exercise library options is not extensive — I instead opted to use the essential exercises and not create a large list as it addresses the main idea while not being sidetracked on how many options there are. The tips system was also implemented more as additional information that the user manually looks for, rather than an automated engine pushing personalised tips to the user, and additionally it also isn't an extensive list. These are some of my system flaws but I could have implemented another API or local database to store a large number of different exercises, as well as for tips, but I would have additionally made tips generated to target areas where data points and logged data on the analytics page suggested a decline across all the areas displayed — all features I would have done given more time.

### 5.2 Evaluation Against Objectives

**Requirement: Website combining diet tracking and exercise in one accessible location**
Met. The program successfully combines exercise and diet tracking into one platform, making it very accessible and easy to use. Diet logging is handled through the FatSecret API, which returns calories and protein for any food searched by name and mass, whilst exercise logging captures both gym sessions (sets, reps, weight) and cardio (type, duration) — all within the same application. Users no longer need to switch between separate apps, which was one of the core frustrations identified in both my client interview and survey research.

**Requirement: User-friendly menu where the different pages are easy to navigate**
Met. The program has a general user-friendly UI with a navbar for overall easy navigation, giving the user a consistent way to reach any part of the site from any page. Navigation is anchored to a central dashboard that links clearly to all sections, and flash messages guide the user on both successful actions and errors so they always know what the system has done. The exercise log also dynamically filters the available exercises based on the type selected — showing only gym or only cardio options — which removes a potential point of confusion that existed in freeworkoutlog. My client noted in her feedback that she found it easy to find what she needed without getting lost, in contrast to the old website she used where she could never remember how to navigate.

**Requirement: Able to log different forms of exercises and not just gym**
Met. The exercise log supports both gym-based strength training (sets, reps, weight) and cardio exercises (type, duration), with the exercise type dynamically filtering the available exercises so only relevant options are shown. This means a user cannot accidentally log a cardio exercise under gym or vice versa, keeping the data accurate and the calorie estimation reliable.

**Requirement: Able to specify user's goal for fitness and diet tracker to target**
Partially met. The program has the ability to create and track goals — users can set a goal type, a target value, a unit and a target date, and track their current progress towards it as a percentage, displayed as a bar chart on the analytics page. However, the system does not yet automatically adjust diet or exercise suggestions based on the goal set — a user targeting fat loss is not shown a recommended calorie deficit, and a user targeting muscle gain is not prompted to hit a higher protein target. This would require the goals system to feed into the tips and analytics pages, which is something I would build with more time and is discussed further in section 5.4.

**Requirement: Sign up and log in to an account for personal data to be saved**
Met. The program implements user accounts so logged data and history can be saved and viewed whenever connected to Wi-Fi. Accounts are created with secure login using validation and password hashing to further secure data if intercepted or hacked — passwords are hashed using pbkdf2:sha256 via Werkzeug before being stored, meaning plain text passwords are never held in the database. All logged data is tied to the user's account via a userId foreign key, and persists across sessions so nothing is lost on logout.

**Requirement: Show daily and long-term macro-nutrient progress through a database**
Met. The program integrates a food macro database through the FatSecret API that enables logging of vast types of food — the user searches any food by name and specifies the mass in grams, and the system automatically retrieves and logs the corresponding calories and protein. This data feeds into the analytics page, which generates four progress graphs viewable across daily, weekly, monthly and yearly time periods, making it easy to track macro-nutrient progress across multiple time scales. This creates a platform that makes tracking progress visual, as highlighted in my research.

**Requirement: Suggest supplements with information and research supplied**
Met. The science-backed tips and advice system includes a tip specifically on creatine supplementation — covering the recommended daily dose, the option of a loading phase, evidence-based benefits including improved strength, muscle mass and recovery, and a cited sports nutrition source. This is accessible on the tips page and can be found by searching "creatine" or filtering to the diet category.

**Requirement: Provides advice and tips on diet and exercise with science behind it**
Met. The program provides a science-backed tips and advice system covering both exercise and diet. Tips include guidance on progressive overload, emphasising the eccentric phase of a lift for muscle hypertrophy, rest and recovery, protein targets for muscle building, hydration and whole food nutrition — each with a cited source. Users can load a random tip, filter by exercise or diet category, or search by keyword, making the advice easy to find and relevant to what the user is currently focusing on.

**Requirement: Graphs to display user statistics and progress**
Met. The program generates four progress graphs viewable across daily, weekly, monthly and yearly time periods, creating a platform that makes tracking progress visual — as highlighted in my research, where 18 of 20 interviewees said graphs would make their progress more tangible and motivating. The charts cover weight history, strength progression per exercise, calorie balance (calories consumed vs. calories burnt) and protein intake, giving the user a broad and clear picture of their progress across all the key areas the program tracks.

**Requirement: Workout preset system**
Met. The workout preset system removes the mundane repetitive task of logging the same exercises again and again if the user's sessions are consistently the same. Users can create a named workout (e.g. "Push Day"), add exercises to it with their usual sets, reps and weight, and then use that preset to log a full session in one go — with the option to adjust individual values for that session without changing the saved preset. This was not an explicit user requirement from section 1.3.4 but emerged as a natural quality-of-life feature from the design process.
